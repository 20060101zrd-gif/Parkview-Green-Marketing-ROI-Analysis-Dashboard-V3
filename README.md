...
智能诊室的诊断卡片**完全由本地规则引擎动态生成**——卡片的标题、严重程度徽章（严重/预警/信息）、左边色点（红/黄/蓝）、建议动作、pct 数值均根据当前数据实时计算，100% 确定性，每次刷新结果一致。流程为：

1. **本地规则引擎**分析当前 KPI、客群、券种、滞后数据 → 遍历 9 个诊断维度，与硬编码阈值比对 → 输出**场景标签**（如 `parking_over_70`、`roi_below_10`、`high_green_cohort` 等 14 个标签）+ 严重程度
2. 本地 `SCENARIO_MAP` 查表 → 得到确定性的 `effect + pct`
3. `_build_rec_text()` 模板引擎 → 生成每条建议的**标题、正文、严重度标签、3 步 how_to 操作指南、采纳按钮参数**
4. （LLM 模式下）DeepSeek 基于诊断结果写一段 executive_summary 总结文字——不参与卡片生成

卡片数量和内容随数据变化：停车券占比 < 30% 时不会出现"资源错配"卡；没有 RED 耗损客群时不会出现"熔断"建议。每张卡均可一键「采纳建议」进入模拟模式。
...
**第四步 · DeepSeek 生成文字解读**：基于诊断卡片的结果和变换后的数值，用自然语言写 **executive_summary** 和 **top_finding** 两段叙述文字。卡片标题、严重度、建议内容、pct 数值**全部由本地引擎生成**，DeepSeek 只负责写总结性描述。如 API 不可用，本地引擎自动生成替代文字。
...
| 职责 | 谁来做 |
|:---|:---|
| 诊断卡片（场景检测、严重度、effect、pct、标题、正文、how_to） | 本地规则引擎 100% |
